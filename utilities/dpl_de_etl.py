...
def load_daily_volume_data(daily_volume_file_path):
    ...
    for data_type in daily_volume_file_path:
        # Duplicate Records in excel files
        if not ((data_type == '2023') or (data_type == '2024')):
            daily_volume_df_list.append(pd.read_excel(daily_volume_file_path[data_type], header=0))
        elif data_type == '2023':
            df_2023 = pd.read_excel(daily_volume_file_path[data_type], header=0)
            df_2023['Data Date '] = pd.to_datetime(df_2023['Data Date '])
            df_2023 = df_2023[df_2023['Data Date ']<'2024-01-01'].copy()
            daily_volume_df_list.append(df_2023)
        else:
            df_2024 = pd.read_excel(daily_volume_file_path[data_type], header=0)
            df_2024['Data Date '] = pd.to_datetime(df_2024['Data Date '])
            df_2024 = df_2024[df_2024['Data Date ']>='2024-01-01'].copy()
            daily_volume_df_list.append(df_2024)
    daily_volume_df = pd.concat(daily_volume_df_list, ignore_index=True)

    return daily_volume_df


def load_historical_hourly_volume(hourly_volume_file_path_sales_hist, hourly_volume_file_path_generation_hist):
    hourly_volume_df_list = []
...
def process_hourly_volume(hourly_volume_df, edc_name):
    ...
    for date_type in ['SALES DATA', 'GENERATION DATA']:
        for type in ['GS-P', 'LG-S', 'MG-S', 'RSCI']:
            for market in ['DPL DE ALT', 'DPL DE SOS', 'DPL DE ELIG']:
                ...
                hourly_volume_data = {
                    ...
                    'EGS_HourlyVolume': sub_df_melted['volume']/1000 if market == 'DPL DE ALT' else 0,
                    'Default_HourlyVolume': sub_df_melted['volume']/1000 if market == 'DPL DE SOS' else 0,
                    'Eligible_HourlyVolume': sub_df_melted['volume']/1000 if market == 'DPL DE ELIG' else 0,
                    'VolumeComment': ''
                }
...
def process_daily_volume(daily_volume_df, edc_name):
    ...
    for service_type in df['Service Type'].unique():
        ...
        if service_type == 'DEG':
            customer_class = 'GSP'
        elif service_type == 'DEL':
            customer_class = 'LGS'
        elif service_type == 'DEM':
            customer_class = 'MGS'
        else:
            customer_class = 'RSCI'

        daily_volume_data = {
            'FlowDate': sub_df['FlowDate'].dt.strftime('%Y-%m-%d'),
            'EDCName': edc_name,
            'CustomerClass': customer_class,
        ...
        customer_count_data = {
            'FlowMonth': sub_df['FlowDate'].dt.strftime('%Y-%m-01'),
            'EDCName': edc_name,
            'CustomerClass': customer_class,
            'VolumeType': 'CustomerCount',
            'EGS_MonthlyVolume': 0,
            'Default_MonthlyVolume': sub_df['SOS COUNT'],
            'Eligible_MonthlyVolume': sub_df['Eligible Count'],
            'VolumeComment': ''
        }
...
def handle_missing_value(hourly_volume_processed, edc_name):
    # Fill in missing hourly volume on 2022-10-31 with the theoretical loss factor between wholesale_Derated and retail_premise
    loss_factor_dict = {
        'RSCI': 1.07438,
        'MGS': 1.07438,
        'LGS': 1.07438,
    ...
    hourly_volume_missing_list = []
    for customer_class in hourly_volume_processed['CustomerClass'].unique():

        for hour in pd.date_range(start="2022-10-31 04:00:00+00:00", end="2022-11-01 03:00:00+00:00", freq='H'):

            EGS_HourlyVolume = hourly_volume_processed.loc[(hourly_volume_processed['CustomerClass'] == customer_class)
                                                           & (hourly_volume_processed['VolumeType'] == 'Wholesale_Derated')
                                                           & (hourly_volume_processed['Datetime_beginning_utc'] == hour), 'EGS_HourlyVolume'].values[0] / loss_factor_dict[customer_class]

            Default_HourlyVolume = hourly_volume_processed.loc[(hourly_volume_processed['CustomerClass'] == customer_class)
                                                               & (hourly_volume_processed['VolumeType'] == 'Wholesale_Derated')
                                                               & (hourly_volume_processed['Datetime_beginning_utc'] == hour), 'Default_HourlyVolume'].values[0] / loss_factor_dict[customer_class]

            missing_df = pd.DataFrame({
                'Datetime_beginning_utc': [hour],
                'EDCName': edc_name,
                'CustomerClass': customer_class,
                'VolumeType': 'Retail_Premise',
                'EGS_HourlyVolume': EGS_HourlyVolume.round(3),
                'Default_HourlyVolume': Default_HourlyVolume.round(3),
                'Eligible_HourlyVolume': EGS_HourlyVolume.round(3) + Default_HourlyVolume.round(3),
                'VolumeComment': ''
            })
            hourly_volume_missing_list.append(missing_df)
    hourly_volume_missing_list.append(hourly_volume_processed)
    hourly_volume_processed = pd.concat(hourly_volume_missing_list, ignore_index=True)
    hourly_volume_processed = hourly_volume_processed.sort_values(by=['Datetime_beginning_utc', 'CustomerClass', 'VolumeType'], ignore_index=True)
...
def main(base_path):
    ...
    hourly_volume_file_path_sales = {
        '2019_sales': f'{base_path}/DPL DE SOS Sales 2019.xlsx',
        '2020_sales': f'{base_path}/DPL DE SOS Sales 2020.xlsx',
        '2021_sales': f'{base_path}/DPL DE SOS Sales 2021.xlsx',
        '2022_sales': f'{base_path}/DPL DE SOS Sales 2022.xlsx',
        '2023_sales': f'{base_path}/DPL DE SOS Sales 2023.xlsx',
    }

    hourly_volume_file_path_generation = {
        '2019': f'{base_path}/DPL DE SOS Generation 2019.xlsx',
        '2020': f'{base_path}/DPL DE SOS Generation 2020.xlsx',
        '2021': f'{base_path}/DPL DE SOS Generation 2021.xlsx',
        '2022': f'{base_path}/DPL DE SOS Generation 2022.xlsx',
        '2023': f'{base_path}/DPL DE SOS Generation 2023.xlsx',
    }

    daily_volume_file_path = {
        '2019': f'{base_path}/2019 Delmarva DE CPLC, NSPLC, and customer counts.xlsx',
        '2020': f'{base_path}/2020 Delmarva DE CPLC, NSPLC, and customer counts.xlsx',
        '2021': f'{base_path}/2021 Delmarva DE CPLC, NSPLC, and customer counts.xlsx',
        '2022': f'{base_path}/2022 Delmarva DE CPLC, NSPLC, and customer counts.xlsx',
        '2023': f'{base_path}/2023 Delmarva DE CPLC, NSPLC, and customer counts.xlsx',
        '2024': f'{base_path}/2024 Delmarva DE CPLC, NSPLC, and customer counts.xlsx',
    }
    ...
    # Fill in missing values in hourly volume on 2022-10-31
    hourly_volume_processed = handle_missing_value(hourly_volume_processed, edc_name)
...
if __name__ == "__main__":
    base_path = 'C:\\Users\\5DIntern3_2024\\Work\\DPL_DE'
    main(base_path)
